...
export const timeEntriesApi = {
  ...
  async getAnalytics(userId: string) {
    // This is a placeholder; real analytics would be done via SQL views or RPC
    const { data, error } = await supabase.rpc('get_time_analytics', { user_id: userId })
    if (error) throw error
    return data
  }
}
...
export const expensesApi = {
  ...
  async getAnalytics(userId: string) {
    // This is a placeholder; real analytics would be done via SQL views or RPC
    const { data, error } = await supabase.rpc('get_expense_analytics', { user_id: userId })
    if (error) throw error
    return data
  }
}

// Combined analytics (profitability, etc.)
export const analyticsApi = {
  async getCombined(userId: string) {
    // This is a placeholder; real analytics would be done via SQL views or RPC
    const { data, error } = await supabase.rpc('get_combined_analytics', { user_id: userId })
    if (error) throw error
    return data
  }
}